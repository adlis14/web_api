...
class Pasien extends REST_Controller {
    ...
    public function index_get(){
        $id = $this->get('id');
        if($id != ''){
            $data_mhs = $this->Pasien_model->getPasien($id);
        }else{
            $data_mhs = $this->Pasien_model->getPasien();
        }
        if($data_mhs == true){
          $this->response(array(
            'status'  => 'true',
    ...
    public function tambah_post(){
        if($this->Pasien_model->tambahDataPasien($_POST) > 0) {
          $this->response(array(
            'status'  => 'true',
            'code'    => '200',
    ...
    public function ubah_post(){
      if($this->Pasien_model->ubahDataPasien($_POST) > 0) {
        $this->response(array(
          'status'  => 'true',
          'code'    => '200',
    ...
    public function hapus_get($id){
      if($this->Pasien_model->hapusDataPasien($id)) {
         $this->response(array(
          'status'  => 'true',
          'code'    => '200',
