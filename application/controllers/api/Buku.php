...
class Buku extends REST_Controller {
    ...
    public function index_get(){
        $id = $this->get('id');
        if($id != ''){
            $data_mhs = $this->Buku_model->getBuku($id);
        }else{
            $data_mhs = $this->Buku_model->getBuku();
        }
        if($data_mhs == true){
          $this->response(array(
            'status'  => 'true',
    ...
    public function tambah_post(){
        if($this->Buku_model->tambahDataBuku($_POST) > 0) {
          $this->response(array(
            'status'  => 'true',
            'code'    => '200',
    ...
    public function ubah_post(){
      if($this->Buku_model->ubahDataBuku($_POST) > 0) {
        $this->response(array(
          'status'  => 'true',
          'code'    => '200',
    ...
    public function hapus_get($id){
      if($this->Buku_model->hapusDataBuku($id)) {
         $this->response(array(
          'status'  => 'true',
          'code'    => '200',
